...
class CooGeneralTask(energyweb.Logger, energyweb.Task):
    ...
    def _log_measured_energy(self):
        ...
        try:
            ...
            self.console.info(self.msg_success.format(energy_data, block_number))
        except Exception as e:
            self.error_log.exception(self.msg_error.format(self.task_config.energy_meter.__class__.__name__, e))
...
class MyApp(energyweb.dispatcher.App):
    ...
    def configure(self):
        try:
            ...
            [self.add_task(CooProducerTask(task_config=producer, polling_interval=interval)) for producer in
             app_config.production]
            [self.add_task(CooConsumerTask(task_config=consumer, polling_interval=interval)) for consumer in
             app_config.consumption]
        except energyweb.config.ConfigurationFileError as e:
            print('Error in configuration file: {}'.format(e))
        except Exception as e:
